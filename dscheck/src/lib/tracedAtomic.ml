...
let do_run init_func init_schedule =
  ...
  | (run_proc, run_op, run_ptr) ->
    { procs; enabled = current_enabled; run_proc; run_op; run_ptr; 
    backtrack = IntSet.empty }

let rec explore ?depth_limit func state clock last_access =
  let s = last_element state in
  List.iter (fun proc ->
      let j = proc.proc_id in
      let i = Option.bind proc.obj_ptr (fun ptr -> IntMap.find_opt ptr last_access) 
  ...
  if IntSet.cardinal s.enabled > 0 then begin
    ...
    if List.length state < Option.value depth_limit ~default:Int.max_int then (
      while IntSet.(cardinal (diff s.backtrack !dones)) > 0 do
        ...
        explore ?depth_limit func statedash new_clock new_last_access
      done)
  end
