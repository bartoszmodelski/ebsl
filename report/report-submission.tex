...
This section describes the implementation details of developed schedulers and, in particular, underlying lock-free algorithms. 


\section{Core data structure}

Lock-free data structures tend to be difficult to use in the real-world. Firstly, any assumptions about naive data structure are off once it has been modified to use fine-grained concurrency. For example, it might be outright infeasible to implement a size() method for a lock-free linked list. This makes the trade-off space far more complex and points in that space (algorithms) much more varied than those of non-synchronized data structure.

Secondly, the effort required to add new features increases as a function of current complexity. From a theoretical point of view, when working with single-threaded code, developer implementing a new function can focus solely on its output space to determine whether it will fit well with other code. Further, OCaml strives to let user specify types, which are isomorphic to the valid state space. Thus, since invalid states are not representable, the new function is correct and fits well with other code as long as it also follows requirements. This could not be further from the truth in the world of fine-grained concurrency. With lock-free operations, a new function has to be made safe to execute concurrently with entirety of existing codebase. Even if the function is trivial, it could stop some of the existing logic to linearize. 

Therefore, this thesis relies on the lock-free queue described by \todo{PAPER A}, which uses a requires a FAD and a CAS operation per insert and removal. While this algorithm is not as fast as existing FAD-only queues, it has a number of other benefits:
\begin{itemize}
...
\section{Relaxation to SPMC Queue}
\section{SPMC Queue with stealing}
\section{SPMC Stack with FIFO stealing}
\section{Sharded Queue}
\section{Overflow avoidance}
