...
yielding, promises, overflow queue, short-lived tasks, long-lived tasks, lots of tasks at any given point in time

very concurrent - queue size important 

not perfectly optimized, mostly a convenient implementation using available primivites



Details would vary over time, e.g. number of calls, numbre of calls in a mixer (conference), length of calls, ratio of codecs, amount of signalling, traffic routed directly between parties (VoIP-to-VoIP) etc. But it would be great to have a robust scheduling baseline! 
