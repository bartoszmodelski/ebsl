...
\begin{figure}
    ...
    \caption{50 years of evolution of processors. The single-core performance sees diminishing returns but during the last 15 years CPU manufacturers turned to multicore architectures. Figure generated using \ref{karlrupp48:online}.}
   \label{fig:50yrs}
\end{figure}
...
This has not been explicitely handled in the current implementation but both approaches are feasible. There is a question of how important is such a measure, given that the proposed IO library (eio \ref{ocamlmul5:online}) tries to use the fast, non-blocking interfaces like io\_uring.
...
This dissertation focused on exploring the trade-offs between various real-world scheduling policies. The core of the work has produced lock-free FIFO and LIFO schedulers, which were then later extended in various ways (\S\ref{section:lifo-heuristics}, \S\ref{section:work-distribution-extensions}) and composed (\S\ref{section:staged-as-composition}). The evaluation used real-world benchmarks to evaluate the implementations focusing on common industrial metrics: throughput and tail latency (\S
