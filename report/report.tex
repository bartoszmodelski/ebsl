...
Performance of staged architecture (\S\ref{section:design-staged}) and overflow queue (\S\ref{section:design_overflow_queue}) depends on the scalability of the underlying queue. Using a lock-free MPMC queue allows two threads to work in parallel, in contrast with locked structure, but it does not guarantee scaling beyond that as shown by figure \todo{fig}. 

Using fine-grained concurrency does not  
